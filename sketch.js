...
function draw() {
  ...
  if(gameState=="stand-by"){
    textSize(18);
    fill("red");
    text("Presiona la tecla Espacio para continuar",40,140);
    fill("cyan");
    text("Usa W, A, S y D o las flechas para moverte",35,176);
    fill("white");
    text("¡Evita todos los asteroides que puedas!",45,212);
    ship.visible=false;
  ...
  if(gameState=="defeat"){
    textSize(18);
    fill("red");
    text("¡Derrota!",175,140);
    fill("yellow");
    text("Presiona la tecla Espacio para reiniciar",45,176);
    ship.visible=false;
    statsHolder.visible=false;
    hull_show.visible=false;
    hull_dis.visible=false;
  ...
  if(gameState=="play"){
    textSize(18);
    fill("white");
    text("Puntaje Actual: " + score, 225,50);
    text("Mejor Puntaje: " + highscore, 225,70);
    textSize(12);
    fill("blue");
    text(shield, 64,265);
    fill("white");
